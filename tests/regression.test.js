...
const productDerivative = app.buildFormulaModel("differentiate (x^2)*(x^3)");
assert(productDerivative.solution.expression === "(2*x)*x^3+x^2*(3*x^2)", "expected product rule derivative");
assert(productDerivative.algorithm.includes("product"), "expected derivative algorithm to mention product rule");
assert(app.buildGroundLevelBreakdown(productDerivative).join("\n").includes("Product rule"), "expected product rule ground explanation");
const quotientDerivative = app.buildFormulaModel("differentiate (x^2)/(x+1)");
assert(quotientDerivative.solution.expression === "((2*x)*(x+1)-x^2*1)/(x+1)^2", "expected quotient rule derivative");
assert(app.buildGroundLevelBreakdown(quotientDerivative).join("\n").includes("Quotient rule"), "expected quotient rule ground explanation");
const chainDerivative = app.buildFormulaModel("differentiate (2x+1)^3");
assert(chainDerivative.solution.expression === "(3*(2*x+1)^2)*2", "expected chain rule derivative");
assert(app.buildGroundLevelBreakdown(chainDerivative).join("\n").includes("Chain rule"), "expected chain rule ground explanation");
const logExpDerivative = app.buildFormulaModel("differentiate ln(x)+exp(x)");
assert(logExpDerivative.solution.expression === "1/x+exp(x)", "expected log and exponential derivative rules");
assert(app.generateCode(logExpDerivative, "javascript").includes("Math.exp"), "expected exp conversion in generated JS");
...
const definiteIntegral = app.buildFormulaModel("integrate 2x from 0 to 3");
assert(definiteIntegral.solution.family === "definite_integral", "expected definite integral family");
assert(definiteIntegral.solution.expression === "9", "expected definite integral value");
assert(definiteIntegral.pseudocode.includes("upper_value"), "expected definite integral pseudocode");
assert(app.buildGroundLevelBreakdown(definiteIntegral).join("\n").includes("do not add + C"), "expected definite integral ground explanation");
const linearSolve = app.buildFormulaModel("2x + 3 = 11");
assert(linearSolve.solution.family === "linear_equation", "expected linear equation solver family");
